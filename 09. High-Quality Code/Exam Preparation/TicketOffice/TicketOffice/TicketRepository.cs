...
namespace TicketOffice
{
    public class TicketRepository : ITicketRepository
    {
        ...
        public string AddAirTicket(string flightNumber, string from, string to, string airline, DateTime dateTime, decimal price)
        {
            ...
            var result = this.AddTicket(ticket, ticket.Type);
            return result;
        }
        ...
        public string DeleteAirTicket(string flightNumber)
        {
            ...
            var result = this.DeleteTicketByUniqueId(ticket.UniqueKey, ticket.Type);
            return result;
        }
        ...
        private string AddTicket(Ticket ticket, TicketType type)
        {
            string key = ticket.UniqueKey;
            if (this.ticketsByUniqueId.ContainsKey(key))
            {
                return Constants.Duplicate + type.ToString().ToLower();
            }
            ...
            return type + Constants.Created;
        }

        private string DeleteTicketByUniqueId(string uniqueKey, TicketType type)
        {
            if (this.ticketsByUniqueId.ContainsKey(uniqueKey))
            {
                ...
                this.ticketCountByType[ticket.Type]--;
                return type + Constants.Deleted;
            }

            return type + Constants.DoesNotExist;
        }
    }
}
