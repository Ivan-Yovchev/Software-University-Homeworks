...
namespace TravelAgency
{
    ...
    {
        private Dictionary<string, Ticket> ticketsByUniqueId = new Dictionary<string, Ticket>();
        ...
        {
            Ticket ticket = new AirTicket(flightNumber, from, to, airline, dateTime, price);
        ...
        public string DeleteAirTicket(string flightNumber)
        {
            ...
            string result = this.DeleteTicketByUniqueId(ticket.UniqueKey);
            return result;
        }

        public string AddTrainTicket(string from, string to, DateTime dateTime, decimal price, decimal studentPrice)
        {
            Ticket ticket = new TrainTicket(from, to, dateTime, price, studentPrice);
            var result = this.AddTicket(ticket);
            return result;
        }
        ...
        {
            Ticket ticket = new TrainTicket(from, to, dateTime);
            string result = this.DeleteTicketByUniqueId(ticket.UniqueKey);
            return result;
        }
        ...
        {
            Ticket ticket = new BusTicket(from, to, travelCompany, dateTime, price);
            var result = this.AddTicket(ticket);
            return result;
        }
        ...
        {
            Ticket ticket = new BusTicket(from, to, travelCompany, dateTime);
            string result = this.DeleteTicketByUniqueId(ticket.UniqueKey);
            return result;
        }
        ...
        private string AddTicket(Ticket ticket)
        {
            ...
            if (this.ticketsByUniqueId.ContainsKey(key))
            {
                return Constants.DuplicateTicket;
            }

            this.ticketsByUniqueId.Add(key, ticket);
        ...
        private string DeleteTicketByUniqueId(string uniqueKey)
        {
            if (this.ticketsByUniqueId.ContainsKey(uniqueKey))
            {
                var ticket = this.ticketsByUniqueId[uniqueKey];
                this.ticketsByUniqueId.Remove(uniqueKey);
                string fromToKey = CreateFromToKey(ticket.From, ticket.To);
            ...
            return Constants.TicketNonExistent;
        }
    }
}
